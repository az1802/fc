...
function formatFoodProps(foodItem) {
  ...
  combosList &&
    combosList.forEach((comboItem, index) => {
      let groupTemp = {};
      let basePrice = parseFloat(comboItem.price || 0);
      groupTemp.name =
        comboItem.groupName || (index == 0 ? '固定菜' : '可选分组' + index);
      addPropsGroupArr(propsGroupArr, groupTemp.name);
  ...
  try {
    ...
    foodList.forEach((foodItem) => {
      ...
      if (categoriesObjTemp[categoryId]) {
        categoriesObjTemp[categoryId].push(
    ...
    categories = requestMenuData.foodCategories.map((categoryItem) => {
      ...
      categoryData.foods =
        (categoriesObjTemp[categoryId] &&
          categoriesObjTemp[categoryId].reduce((res, goodItem) => {
            if (goodItem && !goodItem.hide) {
              ...
              foodData.picUrl =
                foodData.picUrl &&
                foodData.picUrl.slice(0, foodData.picUrl.indexOf('.jpg')) +
                  '.jpg';
