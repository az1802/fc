...
let baseUrl = "http://zhyx-images.eingdong.com/thumb/thumb_31a5e01ff380fd3d05c3b3b8bbf384.jpg"
let shopInfo = {
  name:"SLOW DOWN中山",
  logo:"",
}
...
async function  handleRequestData(requestMenuData) {
  ...
  try {
    ...
    categories =categoryList.map(categoryItem => {
      ...
      categoryData.foods =categoryItem.children[0].list.reduce((res, goodItem) => {
        if (goodItem) {
          let foodData = {
            name:new String(goodItem.title || ""),
            picUrl: goodItem.img_url_thumb ?  goodItem.img_url_thumb : ""   ,
